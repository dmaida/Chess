...
public class Pawn extends Piece {
    ...
    public ArrayList<Board.Tile> getMoves(int currX, int currY) {
        ...
        if (y > 0 && y < 7) {
            ...
            if (isFirstMove && !chessBoard.Tiles[y - (2 * dir)][x].isOccupied
                    && !chessBoard.Tiles[y - (1 * dir)][x].isOccupied && IsKingProtected(x, y - (2 * dir))) {
                moveList.add(chessBoard.Tiles[y - (2 * dir)][x]);
            }
            if (x > 0 && chessBoard.Tiles[y - (1 * dir)][x - 1].isOccupied
                    && isWhite != chessBoard.Tiles[y - (1 * dir)][x - 1].currentPiece.isWhite
                    && IsKingProtected(x - 1, y - (1 * dir))) {
                moveList.add(chessBoard.Tiles[y - (1 * dir)][x - 1]);
            }
            if (x < 7 && chessBoard.Tiles[y - (1 * dir)][x + 1].isOccupied
                    && isWhite != chessBoard.Tiles[y - (1 * dir)][x + 1].currentPiece.isWhite
                    && IsKingProtected(x + 1, y - (1 * dir))) {
                moveList.add(chessBoard.Tiles[y - (1 * dir)][x + 1]);
            }

...
public class King extends Piece {
    ...
    public ArrayList<Board.Tile> getMoves(int currX, int currY) {
        ...
        // check left side
        if (currX > 0) {
            if ((!chessBoard.Tiles[currY][currX - 1].isOccupied || (chessBoard.Tiles[currY][currX - 1].isOccupied
                    && isWhite != chessBoard.Tiles[currY][currX - 1].currentPiece.isWhite))
                    && isItSafe(currX - 1, currY)) {
                moveList.add(chessBoard.Tiles[currY][currX - 1]);
            }
            if (currY > 0
                    && (!chessBoard.Tiles[currY - 1][currX - 1].isOccupied
                            || (chessBoard.Tiles[currY - 1][currX - 1].isOccupied
                                    && (isWhite != chessBoard.Tiles[currY - 1][currX - 1].currentPiece.isWhite)))
                    && isItSafe(currX - 1, currY - 1)) {
        ...
        if (currY > 0
                && (!chessBoard.Tiles[currY - 1][currX].isOccupied || (chessBoard.Tiles[currY - 1][currX].isOccupied
                        && (isWhite != chessBoard.Tiles[currY - 1][currX].currentPiece.isWhite)))
                && isItSafe(currX, currY - 1)) {
    ...
    private ArrayList<Board.Tile> castleMove(ArrayList<Board.Tile> moveList) {
        ...
        if (isItSafe(globalX, globalY) && isFirstMove) { // If King is not checked && King's first move
            if (!chessBoard.Tiles[globalY][globalX + (1 * dir)].isOccupied
                    ...
                    && isItSafe(globalX + (2 * dir), globalY)) { // If spaces in between King the Rook are free and it
                                                                 // is the Rook's first turn
    ...
    public boolean isItSafe(int desiredX, int desiredY) {
        ...
        if (pawnAttack(desiredX, desiredY) || diagnalAttack(desiredX, desiredY) || straightAttack(desiredX, desiredY)
                || knightCanAttack(desiredX, desiredY)) {
            returnVal = false;
        }
    ...
    private boolean pawnAttack(int desiredX, int desiredY) {
        if (desiredX + 1 < 8) {
            if (desiredY - 1 >= 0) {
                if (isWhite && chessBoard.Tiles[desiredY - 1][desiredX + 1].isOccupied
                        && chessBoard.Tiles[desiredY - 1][desiredX + 1].getPiece().name.compareTo("b_Pawn") == 0) {
                    return true;
                }
            }
            if (desiredY + 1 < 8)
                if (!isWhite && chessBoard.Tiles[desiredY + 1][desiredX + 1].isOccupied
                        && chessBoard.Tiles[desiredY + 1][desiredX + 1].getPiece().name.compareTo("w_Pawn") == 0) {
                    return true;
                }
        }
        if (desiredX - 1 >= 0) {
        ...
        while (i < 7 && j > 0) {
            ...
            if (chessBoard.Tiles[i][j].isOccupied && (isWhite != chessBoard.Tiles[i][j].currentPiece.isWhite)) {
                if (chessBoard.Tiles[i][j].getPiece().name.contains("King")) {
                    ...
                    if (chessBoard.Tiles[i][j].getPiece().name.contains("Queen")
                            || chessBoard.Tiles[i][j].getPiece().name.contains("Bishop")) {
                        return true;
                    }
        ...
        for (int j = x + 1; j < 8; j++) {
            if (chessBoard.Tiles[y][j].isOccupied && (isWhite != chessBoard.Tiles[y][j].currentPiece.isWhite)) {
                if (chessBoard.Tiles[y][j].getPiece().name.contains("King")) {
                    ...
                    if (chessBoard.Tiles[y][j].getPiece().name.contains("Queen")
                            || chessBoard.Tiles[y][j].getPiece().name.contains("Rook")) {
                        return true;
                    }
                }
        ...
        for (int i = y + 1; i < 8; i++) {
            if (chessBoard.Tiles[i][x].isOccupied && (isWhite != chessBoard.Tiles[i][x].currentPiece.isWhite)) {
                if (chessBoard.Tiles[i][x].getPiece().name.contains("King")) {
                    ...
                    if (chessBoard.Tiles[i][x].getPiece().name.contains("Queen")
                            || chessBoard.Tiles[i][x].getPiece().name.contains("Rook")) {
                        return true;
                    }
                }
    ...
    private boolean knightCanAttack(int currX, int currY) {
        ...
        if ((y > 1 && x > 0) && (chessBoard.Tiles[y - 2][x - 1].isOccupied
                && (isWhite != chessBoard.Tiles[y - 2][x - 1].currentPiece.isWhite))) {
        ...
        if ((y > 1 && x < 7) && (chessBoard.Tiles[y - 2][x + 1].isOccupied
                && (isWhite != chessBoard.Tiles[y - 2][x + 1].currentPiece.isWhite))) {
            if (chessBoard.Tiles[y - 2][x + 1].getPiece().name.contains("King")) {
                if (otherKingAttack(currY, currX, y - 2, x + 1)) {
                    return true;
        ...
        if ((y > 0 && x > 1) && (chessBoard.Tiles[y - 1][x - 2].isOccupied
                && (isWhite != chessBoard.Tiles[y - 1][x - 2].currentPiece.isWhite))) {
        ...
        if ((y > 0 && x < 6) && (chessBoard.Tiles[y - 1][x + 2].isOccupied
                && (isWhite != chessBoard.Tiles[y - 1][x + 2].currentPiece.isWhite))) {
            if (chessBoard.Tiles[y - 1][x + 2].getPiece().name.contains("King")) {
                if (otherKingAttack(currY, currX, y - 1, x + 2)) {
                    return true;
        ...
        if ((y < 7 && x < 6) && (chessBoard.Tiles[y + 1][x + 2].isOccupied
                && (isWhite != chessBoard.Tiles[y + 1][x + 2].currentPiece.isWhite))) {
            if (chessBoard.Tiles[y + 1][x + 2].getPiece().name.contains("King")) {
                if (otherKingAttack(currY, currX, y + 1, x + 2)) {
                    return true;
        ...
        if ((y < 6 && x < 7) && (chessBoard.Tiles[y + 2][x + 1].isOccupied
                && (isWhite != chessBoard.Tiles[y + 2][x + 1].currentPiece.isWhite))) {
            if (chessBoard.Tiles[y + 2][x + 1].getPiece().name.contains("King")) {
                if (otherKingAttack(currY, currX, y + 2, x + 1)) {
                    return true;
        ...
        if ((y < 6 && x > 0) && (chessBoard.Tiles[y + 2][x - 1].isOccupied
                && (isWhite != chessBoard.Tiles[y + 2][x - 1].currentPiece.isWhite))) {
            if (chessBoard.Tiles[y + 2][x - 1].getPiece().name.contains("King")) {
                if (otherKingAttack(currY, currX, y + 2, x - 1)) {
                    return true;
        ...
        if ((y < 7 && x > 1) && chessBoard.Tiles[y + 1][x - 2].isOccupied
                && (isWhite != chessBoard.Tiles[y + 1][x - 2].currentPiece.isWhite)) {
            if (chessBoard.Tiles[y + 1][x - 2].getPiece().name.contains("King")) {
                if (otherKingAttack(currY, currX, y + 1, x - 2)) {
                    return true;
